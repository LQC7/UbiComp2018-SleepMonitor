...
%\setlength{\belowcaptionskip}{-9pt}
      \includegraphics[width=0.87\linewidth,height=4.8cm]{Figures/BodyPosture.pdf}
...
\begin{figure}[!t]
  ...
  \subfigure[Supine]{
    \label{fig:Supine}
    \includegraphics[width=0.47\linewidth,height=2cm]{Figures/Supine.pdf}}
  \hfill
  \subfigure[Left Lateral]{
    \label{fig:LeftLateral}
    \includegraphics[width=0.47\linewidth,height=2cm]{Figures/LeftLateral.pdf}}
   \subfigure[Right Lateral]{
    \label{fig:RightLateral}
    \includegraphics[width=0.47\linewidth,height=2cm]{Figures/RightLateral.pdf}}
  %\hspace{1in}
  \hfill
  \subfigure[Prone]{
    \label{fig:Prone}
    \includegraphics[width=0.47\linewidth,height=2cm]{Figures/Prone.pdf}}
...
The hand position \cite{AskMetaFilter} during sleep discloses some potential health problems and the improper hand position will bring bad results. For instance,  the hand position on the abdomen or head may indicate its uncomfortable state, while the person is usually not aware of that cases; when the hand is put on the chest, the person is more likely to have a nightmare because of the long-term oppression on the heart. {\systemname} can recognize three hand positions, that are on the abdomen, chest or head when the user is in the supine posture, as shown in Fig. \ref{fig:HandPosition}. Note that because in other body postures, the hand is put on the bed at most time, thus we don't consider different hand positions when the body are in other postures.
...
\begin{figure}[!t]
\centering
      \includegraphics[width=0.97\linewidth]{Figures/breathing.pdf}
  \caption{The periodic change of the acceleration signal. (a) Hand on the abdomen. (b) Hand on the chest.}\label{fig:breathing}
\end{figure}

In fact, when people are sleeping in the supine posture, the hands may be on both sides of the body, on the abdomen, chest or head. Obviously, when the hand is put on the abdomen or chest, we can see that the acceleration signals exhibit a periodic fluctuation, as shown in Fig. \ref{fig:breathing}. This is due to the movement of the abdomen and chest caused by breathing. Therefore, we can first use the occurrence of respiratory events to determine whether the hand is put on the body (abdomen or chest) or not.




When one is breathing, the chest movement amplitude is smaller than abdominal movement amplitude. So, we can identify the hand position by setting a proper threshold. However, in different sleep stages, the respiration amplitudes are different. As a result, the threshold based method cannot figure out the case that the chest movement amplitude in the deep stage is small than the abdomen movement amplitude in the light stage. To deal with this problem, we present a novel algorithm to accurately identify the hand position. Our key observation is that before the hand is put on the body, it experiences a movement from the body side to the end position. Thus we can further integrate the coarse-grained hand movement trajectory to determine the hand position at current time.  Fig. \ref{Bodyhand} shows the rotation angle changes when the hand is moving from an initial point beside the body to different positions on the body.
...
In detail, we propose a novel algorithm to accurately detect the hand position. For the detection of hand position near to the head, we observe that the rotation angle calculated by the gyroscope is significantly different from the angles when the hand is on the chest and abdomen. Thus we use the rotation angle as a feature to detect whether the hand is on the head or not. After that, to detect the hand position on the chest or abdomen,  we first calculate the power spectral density (PSD) of the acceleration signal, as shown in Fig. \ref{fig:PSD}. The frequency with the highest peak is the signal's frequency. If the frequency falls within the reasonable breathing frequency range, which is around 20 times per minute (0.3 Hz), it indicates the occurrence of respiration events, that is, the hand is put on the chest or abdomen. After we determine the periodic signa caused by respiratory events, we look for a recent hand movement and estimate its trajectory based on the rotation angle. Finally, the hand position can be identified accurately.
...
\begin{minipage}[t]{0.31\linewidth}\centering
    \includegraphics[width=0.97\linewidth,height=3.7cm]{Figures/LeftToRight1.pdf}\centering
